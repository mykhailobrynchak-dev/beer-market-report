#!/usr/bin/env python3
"""Generate BEER MARKET report (index.html) from Databricks.
...
Ставимо плейсхолдери — агент / користувач замінює:
  BEER MARKET      — точне значення dim_provider_v2.group_name (UPPERCASE як у БД)
  BEER MARKET   — людська назва для логів і JSON
  2023-09-01        — стартова дата monthly даних, формат YYYY-MM-DD
...
FROM main.ng_delivery.delivery_rating_provider_rating_history r
    JOIN main.ng_delivery.dim_provider_v2 p ON r.provider_id = p.provider_id
WHERE p.country_code = 'ua'
  AND p.group_name = '{PARTNER_NAME}'
  AND p.provider_status = 'active'
  AND p.lifecycle_status = 'ready_for_work'
...
FROM main.ng_delivery.delivery_rating_provider_rating_history r
    JOIN main.ng_delivery.dim_provider_v2 p ON r.provider_id = p.provider_id
    LEFT JOIN main.ng_delivery.fact_order_delivery f ON r.order_id = f.order_id
WHERE p.country_code = 'ua'
  AND p.group_name = '{PARTNER_NAME}'
  AND p.provider_status = 'active'
  AND p.lifecycle_status = 'ready_for_work'
